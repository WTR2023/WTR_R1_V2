...
uint8_t board_message_buffer[6]; // 6字节接收数据
Chassis_Message chassis_message = {0};

int Board_Message_Decode(void)
{
    if (board_message_buffer[0] != 0x0B || board_message_buffer[1] != 0x0C || board_message_buffer[4] != 0x0C || board_message_buffer[5] != 0x0B) {
        for (int i = 0; i < 5; i++) {
...
{
    HAL_UART_Receive_DMA(&BOARD_MESSAGE_UART_HANDLE, board_message_buffer, 6);
